...
structure UnicodeDataGC where
  codepointRaw : String
  codepoint : Nat
  gc : GeneralCategory
  deriving Repr, DecidableEq, Inhabited, Nonempty

instance : UnicodeData UnicodeDataGC where
  codepointRaw := fun (d : UnicodeDataGC) => d.codepointRaw
  codepoint := fun (d : UnicodeDataGC) => d.codepoint
...
def summarizeUnicodeData (ucd : Array UnicodeDataGC) : SummaryUCD := Id.run do
